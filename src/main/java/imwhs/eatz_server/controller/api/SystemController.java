...
public class SystemController {
    ...
    public ClientLaunchNoticeResponse getLatestClientLaunchNotice() {
        return new ClientLaunchNoticeResponse(
                ...
                "환영합니다!",
                """
                        EATZ는 가지고 있는 재료와 도구만으로 지금 당장 요리할 수 있는 레시피를 찾아주고, 다른 사람들과 레시피를 공유할 수 있는 소셜 레시피 플랫폼이에요. 또한, 플래너와 체크리스트를 통해 요리 일정 별로 준비해야 할 재료와 도구까지 체계적으로 관리할 수 있어요.
                        ...
                        EATZ 사용 중에 발생한 문제점이나 불편한 부분, 아이디어, 새로운 재료 혹은 도구, 레시피 등록과 같이 제안해주실만한 게 있다면 아래 경로를 통해 개발자에게 의견을 남겨주세요!
                        찾고 계신 재료나 도구가 없는 경우에도 아래 경로를 통해  
