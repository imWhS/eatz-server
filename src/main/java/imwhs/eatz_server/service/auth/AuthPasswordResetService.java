...
public class AuthPasswordResetService {
    ...
    /**
     * 암호를 초기화하려는 계정의 이메일 인증을 요청합니다.
     * <p>
     *     암호를 초기화하기 위한 이메일 인증 토큰을 생성하고, 이메일 인증 토큰을 담은 딥링크를 이메일 주소로 발급합니다.
     * </p>
     * @param email 이메일 주소
     */
    ...
    public void requestEmailVerification(String email) {
        ...
        mailService.sendMail(
                email,
                "EATZ 암호 재설정 링크",
                "암호를 초기화하려면, EATZ 앱이 설치된 iOS 기기에서 아래 링크로 이동하세요.\n\n" +
                        deepLink + "\n\n" +
                        "이 링크는 편지를 보내드린 시간으로부터 " + EMAIL_VERIFICATION_TOKEN_EXPIRATION_MINUTES + "분 동안 사용할 수 있어요.\n\n" +
                        "한 번 이동한 링크는 바로 폐기돼요. 만약 회원님이 암호 초기화를 요청한 적 없다면, 이 편지를 무시하셔도 돼요.");
    ...
    /**
     * 암호를 초기화하려는 계정의 이메일 주소에 암호 초기화 권한을 부여합니다.
     * <ul>
     *     <li> 이메일 인증 토큰의 진위 여부를 판별합니다.
     *          정상 토큰인 경우, 외부의 hijacking 공격을 방어하기 위해 암호 초기화 토큰을 생성한 후 HTTPS 체널(클라이언트)로 발급합니다. </li>
    ...
    public VerifyResetTokenResponse authorizePasswordReset(String emailToken) {
        ...
        String emailTokenKey = generateRedisEmailVerificationTokenKey(emailToken);
        ...
        // UUID 기반의 무작위 값을 가지는 일회용 암호 초기화 토큰을 생성합니다.
    ...
    public void reset(String authorizedToken, String newPassword) {
        ...
        // 암호 재설정이 완료되면, Redis에서 이메일 인증 토큰을 삭제합니다.
        redisService.delete(authorizedTokenKey);

        mailService.sendMail(
                email,
                "EATZ 계정 암호 재설정 완료 알림",
                 email + " 이메일 주소를 사용 중인 EATZ 계정의 암호가 재설정됐어요.\n\n" +
                         "이제부터 새 암호를 이용해 EATZ에 로그인해주세요.\n\n" +
                         "만약 회원님이 암호 초기화를 요청한 적이 없다면, 빠르게 고객 센터에 문의해주세요."
        );
    }
