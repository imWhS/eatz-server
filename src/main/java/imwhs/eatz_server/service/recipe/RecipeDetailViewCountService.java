...
@Service
public class RecipeDetailViewCountService {
    ...
    /**
     * Redis에 저장된 레시피 별 임시 조회 수 증분량을 Recipe 엔티티의 viewCount에도 반영한 후 Redis에서 삭제합니다.
     */
    @Transactional(rollbackFor = Exception.class)
    public void flushViewCountIncrements() {
        log.info("레시피 별 조회 수 증분량 flush를 시작할게요.");

//        String pattern = generateRedisViewersCountPerIntervalKeyPattern();
//        Set<String> keys = redisService.getKeysByPattern(pattern);
//        for (String key : keys) {
//            long recipeId = Long.parseLong(key.split(":")[1]);
//            int viewCount = redisService.getAsInt(key);
//            log.info("레시피의 조회 수를 flush 할게요. | 레시피 ID: {} | 증분량: {}", recipeId, viewCount);
//            recipeRepository.addViewCount(recipeId, viewCount);
...
//        }
        ...
        for (String recipeId : recipeIds) {
            ...
            log.info("레시피의 조회 수를 flush 할게요. | 레시피 ID: {} | 증분량: {}", recipeId, viewersIncrement);
            recipeRepository.addViewCount(id, viewersIncrement);
    ...
    /**
     * 레시피의 상세 정보를 요청한 로그인 사용자의 조회 기록을 저장합니다.
     * @param recipeId 레시피의 ID
     * @param userId 로그인 사용자의 ID
     * @param timeZone 타임존(시간대)의 ID
     */
    public void markAsViewedTodayByAuthenticated(
            Long recipeId,
            Long userId,
            String timeZone) {
        userRepository.validateExists(userId);
        String viewerId = "authenticated:" + userId;
        markAsViewedToday(recipeId, viewerId, timeZone);
    }

    /**
     * 레시피의 상세 정보를 요청한 게스트 사용자의 조회 기록을 저장합니다.
     * @param recipeId 레시피의 ID
     * @param deviceId 게스트 사용자의 하드웨어(장치) ID
     * @param timeZone 타임존(시간대)의 ID
     */
    public void markAsViewedTodayByGuest(
            Long recipeId,
            String deviceId,
            String timeZone) {
    ...
    /**
     * 특정 레시피의 상세 정보를 요청한 날짜를 기준으로 사용자의 ID를 Redis의 집합에 추가합니다.
     * 또한, 요청한 날짜를 기준으로 레시피의 상세 정보를 처음 요청한 경우 임시 조회 수 증분량를 증가시킵니다.
     * @param recipeId 레시피의 ID
     * @param viewerId 사용자의 ID
     * @param timeZone 타임존(시간대)의 ID
     */
    private void markAsViewedToday(Long recipeId, String viewerId, String timeZone) {
        recipeRepository.validateExists(recipeId);
        String viewersKey = generateRedisViewersKey(recipeId);

        // Redis의 '레시피의 상세 정보를 조회한 사용자 집합'에 사용자의 ID를 추가합니다.
        long viewers = redisService.addElementToSet(viewersKey, viewerId);

        // 다음 날 자정에 해당 레시피에 대한 조회 관련 기록이 초기화되도록 TTL을 설정합니다.
        ...
        if (isMarked) {
            ...
            setViewCountIncrement(recipeId);
            log.info("사용자의 오늘 날짜 기준 레시피 최초 조회 기록을 저장할게요. | " +
                    "요청한 사용자 ID: {} | " +
                    "레시피 ID: {}", viewerId, recipeId);
        }
    }

    private void setViewCountIncrement(Long recipeId) {
    ...
    private static String generateRedisViewersKey(Long recipeId) {
        return "recipe:view_count:" + recipeId + ":viewers";
    }
    ...
    private static String generateRedisIncrementKey(Long recipeId) {
        return "recipe:view_count:" + recipeId + ":increment";
    }
    ...
    private static String generateRedisIncrementFlushTargetRecipeIdsKey() {
        return "recipe:view_count:increment_flush_target_recipeIds";
    }

    @NonNull
    private static String generateRedisViewersCountPerIntervalKeyPattern() {
        return "recipe:" + "*" + ":viewers:count_per_interval";
    }
