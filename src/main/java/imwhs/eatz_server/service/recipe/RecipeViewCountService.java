...
@Service
public class RecipeViewCountService {
    ...
    public void increaseViewCount(HttpServletRequest request, HttpServletResponse response, Long recipeId) {
        ...
        redisService.increaseValue(REDIS_KEY_PREFIX_RECIPE_VIEW_COUNT_OF + recipeId);
    }

    @Transactional
    @Scheduled(fixedRate = SYNC_VIEW_COUNT_FIXED_RATE)
    public void syncViewCount() {
        Set<String> keys = redisService.getKeys(REDIS_KEY_PREFIX_RECIPE_VIEW_COUNT_OF + "*");
        ...
        for (String key : keys) {
            ...
            Integer viewCount = Integer.parseInt(redisService.getValue(key));
