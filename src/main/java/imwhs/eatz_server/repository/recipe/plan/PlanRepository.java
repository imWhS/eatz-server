...
public interface PlanRepository extends JpaRepository<Plan, Long>, PlanCustomRepository {
    ...
    @Query("""
        ...
        where p.user = :user and p.scheduledAt between :startDate and :endDate
    """)
    List<ChecklistItemDto> findChecklistByUserAndDateRange(@Param("user") EatzUser user,
                                                           @Param("startDate") LocalDate startDate,
                                                           @Param("endDate") LocalDate endDate);

    @Query("""
        select new imwhs.eatz_server.dto.plan.ChecklistItemDtoN(
            new imwhs.eatz_server.dto.recipe.RecipeItemBasicDto(
                p.recipe.id,
                p.recipe.title,
                p.recipe.imageUrl,
                p.recipe.cookingTime,
                p.recipe.prepTime,
                (select cast(count(rt) as long) from Rating rt where rt.recipe = p.recipe and rt.deletedAt is null),
                coalesce((select cast(avg(rt.score) as double) from Rating rt where rt.recipe = p.recipe and rt.deletedAt is null), 0.0)
            ),
            new imwhs.eatz_server.dto.ingredient.IngredientDto(ir.ingredient.id, ir.ingredient.name),
            case when iu.id is null then true else false end)
        from Plan p
        ...
        where p.user = :user and p.scheduledAt between :startDate and :endDate
    """)
    List<ChecklistItemDtoN> findChecklistByUserAndDateRangeV2(@Param("user") EatzUser user,
                                                              @Param("startDate") LocalDate startDate,
                                                              @Param("endDate") LocalDate endDate);
