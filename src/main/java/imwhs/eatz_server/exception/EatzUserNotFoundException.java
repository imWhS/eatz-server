...
public class EatzUserNotFoundException extends RuntimeException {
    ...
    public EatzUserNotFoundException(Long id, EatzUserRole role) {
        super(String.format(MESSAGE_TEMPLATE_INVALID_ID_ROLE, id, role == EatzUserRole.ROLE_ADMIN ? "관리자" : "사용자"));
    }

    public EatzUserNotFoundException(String email, boolean isEmail) {
        super(String.format(isEmail ? MESSAGE_TEMPLATE_INVALID_EMAIL : MESSAGE_TEMPLATE_INVALID, email));
        }
